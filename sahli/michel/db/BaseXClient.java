 ...
 */
public class BaseXClient implements AutoCloseable{
    /** UTF-8 charset. */
    ...
    public void execute(final String cmd, final OutputStream o) throws IOException {
        ...
        receive(this.in, o);
        this.info = receive();
        if(!ok()) throw new IOException(this.info);
    }
    ...
    public void watch(final String name, final EventNotifier notifier) throws IOException {
        ...
        if(this.esocket == null) {
            ...
            try(final OutputStream os = this.esocket.getOutputStream(); final InputStream is = this.esocket.getInputStream()) {
                receive(this.in, os);
                os.write(0);
                os.flush();
                is.read();
    ...
    public void close() throws IOException {
        send("exit");
        this.out.flush();
        if(this.esocket != null) this.esocket.close();
        this.socket.close();
    }
    ...
    void send(final String s) throws IOException {
        this.out.write((s + '\0').getBytes(UTF8));
    }
    ...
    private void send(final InputStream input) throws IOException {
        ...
        final BufferedOutputStream bos = new BufferedOutputStream(this.out);
    ...
    public class Query {
        ...
        public void bind(final String name, final String value, final String type) throws IOException {
            this.cache = null;
            exec(3, this.id + '\0' + name + '\0' + value + '\0' + type);
        }
        ...
        public void context(final String value, final String type) throws IOException {
            this.cache = null;
            exec(14, this.id + '\0' + value + '\0' + type);
        }
        ...
        public boolean more() throws IOException {
            if(this.cache == null) {
                ...
                while(BaseXClient.this.in.read() > 0) {
                    receive(BaseXClient.this.in, os);
            ...
            if(this.pos < this.cache.size()) return true;
            this.cache = null;
            return false;
        }
        ...
        public String next() throws IOException {
            return more() ? new String(this.cache.set(this.pos++, null), UTF8) : null;
        }
